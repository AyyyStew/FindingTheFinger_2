...
import {
  ...
  type FlyToTarget,
  type MapViewMode,
} from "../components/MapCanvas/MapCanvas";
...
const METHOD_TOOLTIPS: Record<ProjectionMethod, string> = {
  ...
  phate:
    "PHATE — diffusion-based geometry. Preserves both local clusters and global continuous trajectories.",
  isomap:
    "Isomap — geodesic distances on a manifold. Preserves global curved structure and inter-cluster geometry.",
};
...
export function Map() {
  ...
  const [rightPanelTab, setRightPanelTab] = useState<"search" | "tools">(
    "search",
  );
  const [overlays, setOverlays] = useState<MapOverlayOptions>(
    DEFAULT_OVERLAY_OPTIONS,
  );
  ...
  const [compareReferenceId, setCompareReferenceId] = useState<number | null>(
    null,
  );
  ...
  const [compareHoverUnitId, setCompareHoverUnitId] = useState<number | null>(
    null,
  );
  ...
  const unitPositionMap = useMemo(
    () =>
      resolvedData
        ? buildUnitPositionMap(resolvedData)
  ...
  const unitCorpusMap = useMemo(
    () =>
      resolvedData
        ? buildUnitCorpusMap(resolvedData)
        : new globalThis.Map<number, number>(),
    [resolvedData],
  );
  ...
  const compareSelectionSet = useMemo(
    () => new globalThis.Set(compareSelectionIds),
    [compareSelectionIds],
  );
  ...
  const compareHoverPosition = useMemo(() => {
    if (
      compareHoverUnitId == null ||
      !compareSelectionIds.includes(compareHoverUnitId)
    )
      return null;
  ...
  const handleToolsMapClick = useCallback(
    (info: HoverInfo) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      !compareSelectionIds.includes(referenceUnitId)
    )
      return;
  ...
  const selectedUnitLabels = useMemo(() => {
    ...
    if (compareResult) {
      labels[compareResult.reference_unit.id] =
        compareResult.reference_unit.reference_label;
  ...
  const pcaManifest =
    projData && isPcaRunData(projData)
  ...
  return (
    <>
      <section
        className={styles.mobileFallback}
        aria-label="Map screen size notice"
      >
        <div className={styles.mobileFallbackCard}>
          <span className={styles.mobileFallbackKicker}>Map view</span>
          <h2 className={styles.mobileFallbackTitle}>
            Best experienced on larger screens
          </h2>
          <p className={styles.mobileFallbackBody}>
            The full projection map includes high-density WebGL rendering and
            multi-panel controls. For smooth exploration, open this page on
            tablet, laptop, or desktop.
          </p>
          <span className={styles.mobileFallbackCta}>
            Use a larger screen for full map tools
          </span>
        </div>
      </section>
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.canvasWrap}>
          ...
          <div className={styles.canvasInner}>
            ...
            !resolvedData ||
            !resolvedVisibility ||
            projData?.manifest.method !== method ? (
              <div className={styles.centred}>
              ...
              <>
                ...
                />
                <div
                  className={styles.overlayToolbar}
                  aria-label="Map view overlays"
                >
                  {viewMode === "2d" && (
                    <>
                      <button
                ...
                <MapCanvas
                  ...
                  selectedUnitIds={
                    rightPanelTab === "tools" ? compareSelectionSet : null
                  }
                  selectedPositions={
                    rightPanelTab === "tools" ? compareSelectionPositions : null
                  }
                  selectedHoverPosition={
                    rightPanelTab === "tools" ? compareHoverPosition : null
                  }
                  ...
                  >
                    {hoveredUnit ? (
                      ...
                      <div className={styles.tooltipSkeleton}>
                        {resolvedData.unitLabels[String(hover.unitId)] ??
                          `Unit ${hover.unitId}`}
                      </div>
                    )}
                  </div>
