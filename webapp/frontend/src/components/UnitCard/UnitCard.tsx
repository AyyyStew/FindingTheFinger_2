...
export function UnitCard({
  ...
  return (
    <article
      ...
      style={{ '--tx-solid': solid, '--tx-dim': dim, ...style } as React.CSSProperties}
    >
      ...
      <div className={styles.inner}>
        ...
        <div className={styles.badges}>
          {taxonomyRoot && (
            <span className={styles.taxBadge}>{taxonomyRoot.name}</span>
          )}
          <span className={styles.badge}>{unit.corpus_name}</span>
        ...
        {showChildren && (
          <UnitChildren unitId={unit.id} height={unit.height} />
        )}
      </div>
    </article>
  )
